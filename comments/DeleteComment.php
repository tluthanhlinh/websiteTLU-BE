<?php
// D:\App\PHP\xampp\htdocs\webxudoan\backend\api\comments\DeleteComment.php

// Đường dẫn từ DeleteComment.php (trong api/comments/) đến cors.php (trong src/utils/)
require_once '../../src/utils/cors.php';

// Đường dẫn từ DeleteComment.php (trong api/comments/) đến Database_test.php (trong config/Database/)
...
// Chỉ chủ sở hữu bình luận, admin, hoặc manager mới được phép xóa.
// ========================================================================
...
function decodeJwtAndGetUserAuthInfo($token, $db_pdo_connection) {
    // Hàm này giống như trong UpdateComment.php
    if (strpos($token, 'user_auth_token_for_') === 0) {
        ...
        try {
            ...
            error_log("Lỗi truy vấn thông tin người dùng từ token giả (DeleteComment): " . $e->getMessage());
        }
    }
...
if (empty($token) || $currentUserId === null) {
    ...
    echo json_encode(['success' => false, 'message' => 'Unauthorized: Please log in to delete a comment.'], JSON_UNESCAPED_UNICODE);
    exit();
}
...
// Lấy dữ liệu JSON từ request body (chứa ID bình luận cần xóa)
...
if ($commentId === false) {
    ...
    echo json_encode(['success' => false, 'message' => 'ID bình luận không hợp lệ.'], JSON_UNESCAPED_UNICODE);
    exit();
}
...
try {
    ...
    if (!$existingComment) {
        ...
        echo json_encode(['success' => false, 'message' => 'Không tìm thấy bình luận để xóa.'], JSON_UNESCAPED_UNICODE);
        exit();
    }

    // Kiểm tra phân quyền: Chỉ chủ sở hữu, admin, hoặc manager mới được xóa
    if ($currentUserRole !== ROLE_ADMIN && $currentUserRole !== ROLE_MANAGER && $currentUserId !== (int)$existingComment['user_id']) {
        $db->rollBack();
        http_response_code(403); // Forbidden
        echo json_encode(['success' => false, 'message' => 'Forbidden: You do not have permission to delete this comment.'], JSON_UNESCAPED_UNICODE);
        exit();
    }
    ...
    $stmt_delete->bindParam(':id', $commentId, PDO::PARAM_INT);
    ...
    if ($stmt_delete->execute()) {
        ...
        if ($affectedRows > 0) {
            ...
            echo json_encode(['success' => true, 'message' => 'Bình luận đã được xóa thành công.', 'comment_id' => $commentId], JSON_UNESCAPED_UNICODE);
        } else {
            ...
            echo json_encode(['success' => false, 'message' => 'Không tìm thấy bình luận để xóa hoặc bình luận đã bị xóa trước đó.'], JSON_UNESCAPED_UNICODE);
        }
    } else {
        $db->rollBack(); // Hoàn tác transaction
        http_response_code(500); // Internal Server Error
        error_log("Lỗi khi xóa bình luận: " . json_encode($stmt_delete->errorInfo()));
        echo json_encode(['success' => false, 'message' => 'Lỗi khi xóa bình luận: ' . $stmt_delete->errorInfo()[2]], JSON_UNESCAPED_UNICODE);
    }

} catch (PDOException $e) {
    $db->rollBack(); // Hoàn tác transaction
    http_response_code(500);
    error_log("Lỗi PDO khi xóa bình luận: " . $e->getMessage());
    echo json_encode(['success' => false, 'message' => 'Lỗi máy chủ nội bộ khi xóa bình luận.', 'error' => $e->getMessage()], JSON_UNESCAPED_UNICODE);
}
?>